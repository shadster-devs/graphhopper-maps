...
function addSelectedPathsLayer(map: Map, selectedPath: SegmentedPath, displayMode: PathDisplayMode) {
    ...
    selectedPath.segments.forEach((segment, segmentIndex) => {
        ...
        if (segment.mode.toLowerCase() === 'flights' || segment.mode.toLowerCase() === 'flights') {
            const curvedCoordinates = createBezierLineString(coordinates[0], coordinates[coordinates.length - 1]);
            console.log('curvedCoordinates', curvedCoordinates)
            feature.setGeometry(new LineString(curvedCoordinates));
        } else {
